...
Repositori GitHub: anamofficial436-collab/UAS_Khaerul

PENDAHULUAN & ARSITEKTUR CI/CD
Dokumentasi ini disusun untuk memenuhi kriteria penilaian UAS mata kuliah Administrasi Server. Proyek ini mengintegrasikan Web Statis dan Web Dinamis ke dalam satu infrastruktur cloud di AWS EC2 yang diorkestrasikan menggunakan Docker Compose dan dideploy secara otomatis menggunakan GitHub Actions melalui pendekatan modern Continuous Integration & Continuous Deployment (CI/CD).

🌐 Alur Arsitektur Deployment & CI/CD
Push Trigger: Setiap kali perubahan kode dilakukan dan di-push ke branch main, pipeline GitHub Actions akan terpicu secara otomatis.
Paths Filter (Smart Build): Pipeline mendeteksi folder mana yang mengalami perubahan menggunakan paths-filter. Jika hanya folder web-dinamis yang berubah, maka runner hanya akan memproses build untuk komponen tersebut demi menghemat efisiensi waktu eksekusi.
Build & Push Images: GitHub Runner masuk ke Docker Hub menggunakan token kredensial terenkripsi. Kode dikompilasi langsung ke dalam Docker Image terpisah (web-statis dan web-dinamis) dengan memanfaatkan sistem caching GitHub Actions (type=gha), lalu dipublikasikan ke Docker Hub registry.
Deploy via SSH & SCP: Konfigurasi docker-compose.yml serta .env.example ditransfer ke AWS EC2 menggunakan protokol SCP. Pipeline kemudian melakukan koneksi SSH remote ke VM EC2 untuk memperbarui environment, menarik (pull) image terbaru, membersihkan kontainer bentrok, dan menyalakan ulang layanan microservices dalam mode background (detached mode).
